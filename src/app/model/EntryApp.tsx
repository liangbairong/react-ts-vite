...
const I18nEntry = observer((props: I18nEntryProps): JSX.Element => {
    ...
    useEffect(() => {
        fetch('/env.json')
            ...
            .then(() => {
                ...
                if (anchorId) {
                    appStore.getAnchorRegion(anchorId);
                }
            });
    }, []);
    ...
    useEffect(() => {
        setInitDone(false);
        const existenceLocale = ['vi', 'id', 'ms'].includes(currentLocale) ? 'en' : currentLocale;
        const url = '/i18n/' + locales[existenceLocale] + '/index.json?v=' + import.meta.env.VITE_VERSION;
        fetch(url)
            .then((response) => response.json())
            .then((json) => {
                console.log('国际化');
    ...
    useEffect(() => {
        appStore.setIsLogin(!!accessToken);
    }, [accessToken]);
